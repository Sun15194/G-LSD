...
class Box(dict):
    ...
    :param default_box: 与defaultdict类似，返回一个默认值
    :param default_box_attr: 指定默认替换。
        WARNING: 如果这不是默认的'Box'，它将不会是递归的
    :param frozen_box: 创建完成后，不能再修改该框
    :param camel_killer_box: 转换CamelCase为snake_case
    :param conversion_box: 检查接近匹配的键作为属性
    :param modify_tuples_box: 在方框中重新创建带有字典的传入元组
    :param box_it_up: 从一开始递归地创建所有Boxes
    :param box_safe_prefix: 不安全属性的转换框前缀
    :param box_duplicates: 当conversion_box中存在重复项时，"ignore", "error" 或者 "warn"
    :param ordered_box: 保存进入box的keys的顺序
    """
    ...
    def __new__(cls, *args, **kwargs):
        ...
        由于pickle在python 3中的工作方式，我们需要确保尽早创建box配置。
        """
